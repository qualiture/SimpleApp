...
class SalesOrderHeaderMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ESPMContainerFmwk.SalesOrderHeader]?, Error?) -> Void) -> Void)?
    private var entities = [ESPMContainerFmwk.SalesOrderHeader]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salesOrderHeader = entities[indexPath.row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            detailViewController.navigationItem.title = entities[(tableView.indexPathForSelectedRow?.row)!].salesOrderID ?? ""
...
extension SalesOrderHeaderMasterViewController: ESPMContainerEntitySetUpdaterDelegate {
