...
class StockMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var dataService: ESPMContainer<OfflineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ESPMContainerFmwk.Stock]?, Error?) -> Void) -> Void)?
    private var entities = [ESPMContainerFmwk.Stock]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let stock = entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ProductId", value: "\(stock.productID!)")
        return cell
    }
...
extension StockMasterViewController: ESPMContainerEntitySetUpdaterDelegate {
